...
def create_session(body: SessionCreate) -> Any:
    ...
    with get_session() as db:
        session = ChatSession(id=new_id, title=body.title)
        db.add(session)
        db.commit()
        db.refresh(session)
        return SessionResponse(id=session.id, title=session.title, created_at=session.created_at, updated_at=session.updated_at)
...
def list_sessions() -> Any:
    with get_session() as db:
        ...
        return [SessionResponse(id=r.id, title=r.title, created_at=r.created_at, updated_at=r.updated_at) for r in rows]
...
def get_session_info(session_id: str) -> Any:
    with get_session() as db:
        ...
        return SessionResponse(id=sess.id, title=sess.title, created_at=sess.created_at, updated_at=sess.updated_at)
